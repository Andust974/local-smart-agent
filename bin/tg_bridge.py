#!/usr/bin/env python3
import os, json, time, urllib.request, urllib.parse, pathlib, sqlite3
...
def http_get(url: str, params: dict | None = None, timeout: int = 30) -> str:
    if params:
...
def tg_send_document(chat_id: int, file_path: str) -> None:
...
def is_auth(chat_id: int, st: dict) -> bool:
    return chat_id in st.get("authorized", [])

# ---------- agent ----------
def agent_health() -> str:
    return http_get(f"http://127.0.0.1:{AGENT_PORT}/health", {"token": AGENT_TOKEN}, 20)
...
# ---------- dynamic commands (SQLite) ----------
def dyn_db_path() -> pathlib.Path:
    d = BASE / "data" / "tg_bridge"
    d.mkdir(parents=True, exist_ok=True)
    return d / "commands.sqlite"

def dyn_get_command(name: str, workspace: str = "default") -> dict | None:
    try:
        db = sqlite3.connect(str(dyn_db_path()))
        cur = db.execute(
            "SELECT name,type,payload FROM commands WHERE workspace=? AND name=? AND enabled=1 LIMIT 1",
            (workspace, name),
        )
        row = cur.fetchone()
        db.close()
        ...
        try:
            ...
            payload = {}
        return {"name": nm, "type": typ, "payload": payload}
    except Exception as e:
        log(f"dyn_get_command error: {e}")
        return None

def dyn_exec(chat_id: int, text: str) -> bool:
    if not text.startswith("/"):
        return False
    cmd = dyn_get_command(text)
    if not cmd:
        return False
    typ = cmd["type"]
    pl  = cmd["payload"] or {}
    try:
        if typ == "reply_text":
            tg_send(chat_id, str(pl.get("text", "")))
            return True
        if typ == "reply_link":
            tg_send(chat_id, str(pl.get("url", "")))
            return True
        if typ == "call_api":
            path = pl.get("path", "/health")
            params = pl.get("params", {})
            resp = agent_call(path, params)
