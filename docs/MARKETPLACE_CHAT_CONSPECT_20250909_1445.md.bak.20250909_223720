...
## 2) ТЗ (коротко)
- **MVP:** установка одной командой; ядро агента + skills (web_read, report_pack, dashboard); локальный API (`/health`, `/task`/`/report_pack`); простой Dashboard.
...
- Ядро/скрипты (заглушки): `core/agentd.sh`, `core/task_runner.sh`, `bin/report_pack.sh`, `bin/inbox_watcher.sh`, `bin/gen_reports_index.sh` (заглушка), др.
- API (минимально рабочий): `bin/task_api.py` — реализует `GET /health` и `GET /report_pack?count=N&token=...` (кладёт JSON-задачу в `tasks/inbox/`).
...
- `sandbox/dashboard.html` — минимальный дашборд; добавлена кнопка «Собрать бандл (N)» (JS вызывает `/report_pack`).
...
4. **Dashboard**: восстановлен `sandbox/dashboard.html`, добавлена кнопка «Собрать бандл (N)» (prompt → fetch `/report_pack`).  
...
7. **Watcher и бандл-складчик**: `bin/inbox_watcher.sh` (читает JSON-задачи, вызывает `bin/report_pack.sh`), сам `report_pack.sh` собирает ZIP из N последних отчётов.  
...
10. **Проверка**: `/health` вернул `{"status":"ok"}`; `/report_pack?count=2&token=changeme` → создан `tasks/inbox/task_YYYYMMDD_...json`; watcher переместил задачу в `tasks/done/` и создал `reports/bundle_YYYYMMDD_HHMMSS.zip`.

---

## 5) Подтверждённые артефакты и команды
- `curl -s http://127.0.0.1:8766/health` → `{"status":"ok"}`.  
- `curl -s "http://127.0.0.1:8766/report_pack?count=2&token=changeme"` → `{"queued":"tasks/inbox/task_...json"}`.  
...
- Локальный рабочий API (`/health`, `/report_pack`) + systemd unit.  
...
- `bin/inbox_watcher.sh`, `bin/report_pack.sh` — обработка задач и сбор бандлов.  
...
- Реализован минимальный API (`/health`, `/report_pack`) и запущен через systemd.  
